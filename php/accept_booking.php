...
if (isset($_POST['booking_id']) && !empty($_POST['booking_id'])) {
    ...
    // Update the status to 'accepted' in the booking_log table
    $update_query = "UPDATE booking_log SET status = 'accepted' WHERE booking_id = '$booking_id'";
    if (mysqli_query($conn, $update_query)) {
        $response['success'] = true;
        $response['message'] = 'Booking accepted successfully.';
    } else {
        $response['success'] = false;
        $response['message'] = 'Error accepting booking: ' . mysqli_error($conn);
    }
} else {
